<template>
  <q-header elevated class="bg-primary text-white" height-hint="98">
    <q-toolbar>
      ...
      <q-toolbar-title>编辑课程</q-toolbar-title>
    </q-toolbar>
    <q-tabs v-model="tab">
...
import { getCourseRaw, updateCourse } from '../../service/course.js'
...
onMounted(async () => {
  const res = await getCourseRaw(_id)
  course.value = res.course
  arr.value = res.courseArr
})
...
async function submit() {
  await updateCourse(course.value, arr.value)
  router.back()
}
